 ...
 * 描    述：AFC-7快速原型组件程序常数、结构类型等的定义文件
 * 版    本：
...
typedef struct {
    ...
    uint16_t ctlStep;       // Simulink模型中的控制步长计数
    uint8_t  testStep;      // TestCom测控输出步长计数
    
    uint16_t cnt_OutTime;    // 超时检测用时标 (ms)
    uint16_t relTime2PPS;    // 以1PPS为基准的相对时间 (ms)

    uint32_t fcsM33Time;       // 1ms级物理采样步长驱动器 (%4, %20使用)
    
    // 🚨 落实曾总指示：彻底升级为 64 位微秒硬时标，确保 58 万年不溢出
    uint64_t fcsM33Time_us;  // M33核飞控调度时标（单位：微秒）
    uint64_t taskA35Time_us; // A35核任务调度时标（单位：微秒）
} TRealTimeCnt;

typedef struct {
	uint64_t t0_A35_us;   // T0时刻的时间拍照
	uint64_t t0_M33_us;
	bool bIsT0Latched;    // T0时刻拍照记录标志
} TT0Snapshot;
